(ns mvretailops.phase
  "Phase 0->3 staged rollout for the ISIC-4762 'Retail sale of music and
  video recordings in specialized stores' operations-coordination actor.
  ...
  `:flag-inventory-concern` is deliberately ABSENT from every phase's
  `:auto` set, including phase 3 -- a permanent structural fact, not a
  rollout milestone still to come. Flagging an inventory concern always
  needs a human to actually look at it.
  `mvretailops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [mvretailops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-inventory-concern` is a member of
...
(defn gate
  ...
  - `:flag-inventory-concern` is never auto-eligible at any phase, so it
...
(defn verdict->disposition
  "Map a MusicVideoRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
