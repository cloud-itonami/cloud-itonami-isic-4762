(ns mvretailops.sim
  ...
  escalates regardless of phase), then an inventory-concern flag (ALWAYS
  ...
  the permanently-excluded copyright/licensing-dispute-finalization
  scope."
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "store-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :mv-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :mv-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
      ...
      (println r)
      (println "-- human store coordinator approves --")
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "vinyl restock" :quantity 60 :estimated-cost 480.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "boxed 4K collector's-edition catalog" :quantity 200 :estimated-cost 4200.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human store coordinator reviews & approves --")
      (println (approve! actor "t5")))

    (println "\n== flag-inventory-concern store-1 (ALWAYS escalates, even at phase 3) ==")
    (let [r (exec-op actor "t6" {:op :flag-inventory-concern :store-id "store-1"
                                 :patch {:concern "suspected bootleg pressings mixed into a returned vinyl bundle, mis-shipped quantities from last delivery" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "grey-market DVD import" :quantity 40 :estimated-cost 250.0
    ...
    (println "\n== log-sales-record store-1, advisor drifts into copyright/licensing-dispute-finalization scope -> HARD hold, permanent ==")
