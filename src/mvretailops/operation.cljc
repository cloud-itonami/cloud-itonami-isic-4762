(ns mvretailops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (MusicVideoRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MusicVideoRetailGovernor
  ...
  human store operator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mvretailops.advisor :as advisor]
            [mvretailops.governor :as governor]
...
(defn build
  ...
  `mvretailops.store/Store`).
  opts:
    :advisor      -- an `mvretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MusicVideoRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MusicVideoRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
