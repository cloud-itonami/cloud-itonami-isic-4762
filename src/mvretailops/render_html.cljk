(ns mvretailops.render-html
  ...
  (`mvretailops.operation` -> `mvretailops.governor` ->
  `mvretailops.store`) through a scenario adapted from this repo's
  own `mvretailops.sim` demo driver (`clojure -M:dev:run`, confirmed
  ...
  `vendor-2` -- ids that DO match `mvretailops.store/demo-data`, so it
  ...
  (:require [jp-go-dds.skin]
            ...
            [mvretailops.advisor :as advisor]
            [mvretailops.store :as store]
            [mvretailops.operation :as op]
            [langgraph.graph :as g]))

(def ^:private coordinator
  {:actor-id "coord-1" :actor-role :mv-store-coordinator :phase 3})

(defn- exec! [actor tid request]
  (g/run* actor {:request request :context coordinator} {:thread-id tid}))

(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "store-coordinator-1"}}
...
(defn run-demo!
  ...
  high-cost supply-order (ALWAYS escalates -- approved), and an
  inventory-concern flag (ALWAYS escalates -- approved); store-99
  ...
  proposal that drifts into copyright/licensing-dispute-finalization
  scope HARD-holds permanently. Every HARD hold never reaches a human.
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "t3-supply-low" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "vinyl restock" :quantity 60
                                          :estimated-cost 480.0 :vendor-id "vendor-1"}})

    (exec! actor "t4-supply-high" {:op :coordinate-supply-order :store-id "store-1"
                                   :patch {:item "boxed 4K collector's-edition catalog"
                                           :quantity 200
                                           :estimated-cost 4200.0 :vendor-id "vendor-1"}})
    (approve! actor "t4-supply-high")

    (exec! actor "t5-flag" {:op :flag-inventory-concern :store-id "store-1"
                            :patch {:concern "suspected bootleg pressings mixed into a returned vinyl bundle, mis-shipped quantities from last delivery"
                                    :confidence 0.92}})
    ...
    (exec! actor "t8-vendor" {:op :coordinate-supply-order :store-id "store-1"
                              :patch {:item "grey-market DVD import" :quantity 40
...
(def ^:private action-gate-rows
  ...
  ;; (README `Features`, `mvretailops.governor`/`mvretailops.phase`) --
  ...
  ["        <tr><td><code>:log-sales-record</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence</span></td></tr>"
   ...
   "        <tr><td><code>:flag-inventory-concern</code></td><td><span class=\"warn\">ALWAYS human approval &middot; never auto at any phase &middot; surfaces concern only, never finalizes a copyright/licensing dispute</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-4762 &middot; specialized-music-video-retail</title><style>"
     (jp-go-dds.skin/dds+skin)
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Retail sale of music and video recordings in specialized stores (ISIC 4762) — Operator Console</h1>\n"
     "  <span class=\"badge\">read-only sample · governor-gated · inventory concern / high-cost supply always human-approved · HARD holds permanent</span>\n"
     "</header>\n"
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Specialized music/video retail stores</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>mvretailops.store</code> via <code>mvretailops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated from the real actor stack.</p>\n"
     "    <table>\n"
     ...
     "    <p class=\"muted\">A <code>:coordinate-supply-order</code> must name a registered &amp; verified vendor — grey-market / counterfeit media import brokers are the failure mode this gate permanently blocks.</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (MusicVideoRetailGovernor)</h2>\n"
     "    <p class=\"muted\">HARD holds cannot be overridden: store unverified (business registration + retail license), vendor unverified (for supply orders), effect not <code>:propose</code>, and permanently out-of-scope copyright/licensing-dispute finalization (approve/deny/payout/settlement). An unregistered or unverified store or vendor never reaches a human.</p>\n"
     "    <table>\n"
