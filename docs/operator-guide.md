...
1. Register operator, stores and vendors; independently confirm each
   ...
   seeding `mvretailops.store`.
2. Import existing sales/inventory/return and staffing/supply-order
   history.
...
5. Publish a dry-run inventory-concern flag and audit export.
...
- human sign-off for `:flag-inventory-concern` (always) and high-cost
...
governor-bypass resistance, evidence-backed inventory-concern reporting
