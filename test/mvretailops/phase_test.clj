(ns mvretailops.phase-test
  "Unit tests of `mvretailops.phase` rollout logic."
  (:require [clojure.test :refer [deftest is testing]]
            [mvretailops.phase :as phase]))
...
(deftest inventory-concern-holds-when-not-enabled
  (testing ":flag-inventory-concern holds in phases 0-2 (not yet enabled)"
    (doseq [ph [0 1 2]]
      (let [{:keys [disposition]} (phase/gate ph {:op :flag-inventory-concern} :escalate)]
        (is (= :hold disposition)
            (str "phase " ph " has not enabled flag-inventory-concern yet"))))))

(deftest inventory-concern-escalates-when-enabled
  (testing ":flag-inventory-concern ALWAYS escalates when enabled, even if governor says commit"
    (let [{:keys [disposition]} (phase/gate 3 {:op :flag-inventory-concern} :commit)]
      (is (= :escalate disposition)
          "phase 3 must escalate inventory concerns regardless of governor disposition"))))

(deftest inventory-concern-never-in-any-phase-auto-set
  (testing ":flag-inventory-concern must never be a member of any phase's :auto set -- a permanent structural fact, not a rollout milestone"
    (doseq [[ph {:keys [auto]}] phase/phases]
      (is (not (contains? auto :flag-inventory-concern))
          (str "phase " ph " :auto set must never contain :flag-inventory-concern")))))
