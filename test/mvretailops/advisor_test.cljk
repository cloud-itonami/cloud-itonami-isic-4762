(ns mvretailops.advisor-test
  "Unit tests of `mvretailops.advisor` proposal generation."
...
(deftest propose-inventory-concern-shape
  (testing "inventory-concern proposal always escalates"
    (let [p (adv/infer db {:op :flag-inventory-concern
                           :store-id "store-1"
                           :patch {:concern "suspected bootleg pressings in returned bundle"}})]
      (is (= :flag-inventory-concern (:op p)))
      (is (= :propose (:effect p)))
      (is (string? (:summary p))))))
