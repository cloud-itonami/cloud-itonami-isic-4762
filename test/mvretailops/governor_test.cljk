(ns mvretailops.governor-test
  "Pure unit tests of `mvretailops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [mvretailops.advisor :as adv]
            [mvretailops.governor :as gov]
            [mvretailops.store :as store]))

(def store-1 {:store-id "store-1" :name "Vinyl Vault Records & Film" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Media Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Meridian Music & Film Distribution" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Grey-Market Media Import Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine music/video store coordination"
...
(deftest op-outside-allowlist-is-hard
  (testing "an op outside the closed four-op allowlist is a scope violation"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (clean-proposal :finalize-copyright-claim "store-1") s)]
      (is (true? (:hard? verdict)))
      (is (some #{:op-not-allowed} (map :rule (:violations verdict)))))))

(deftest approve-claim-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly approving a copyright claim is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "approved the copyright claim and processed the royalty payment immediately"
                          :confidence 0.99)
...
(deftest deny-claim-finalization-content-is-hard
  (testing "a proposal touching denying a copyright claim is HARD-blocked, same as approval"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "denied the copyright claim before the customer left the store"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))
...
(deftest settle-claim-content-is-hard
  (testing "a proposal touching settling a licensing dispute is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-staffing-operation "store-1")
                          :summary "front-desk staff should settle the licensing dispute at pickup")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest issue-settlement-content-is-hard
  (testing "a proposal touching issuing a copyright infringement settlement is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "issued a copyright infringement settlement at the loading dock")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-inventory-concern-is-not-scope-excluded
  (testing "flagging observed counterfeit/piracy/mis-shipment concerns as an INVENTORY CONCERN (not a claim finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-inventory-concern "store-1")
                         :value {:concern "suspected bootleg pressings mixed into a returned vinyl bundle, mis-shipped quantities from last delivery"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (counterfeit/piracy/mis-shipment) is exactly what this op exists to surface"))))
...
;; "copyright" or "piracy"), which then accidentally matches inside the
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "vinyl restock" :estimated-cost 480.0 :vendor-id "vendor-1"}
                      {})
